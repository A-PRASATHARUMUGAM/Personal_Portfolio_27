      ...
      >
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="font-mono text-xs uppercase tracking-widest text-neutral-400">{project.status}</span>
          </div>
          <h2 id="modal-title" className="text-2xl sm:text-4xl font-light text-white tracking-tight font-mono">
            {project.title}
          </h2>
        </div>
        ...
        {/* Tech Stack Tags */}
        <div className="mb-8">
          <span className="block font-mono text-[9px] uppercase tracking-[0.25em] text-neutral-500 mb-3">SYSTEM TECHNOLOGY</span>
          <div className="flex flex-wrap gap-2">
            {project.technology.map((tech) => (
              <span key={tech} className="font-mono text-xs text-neutral-300 border border-white/10 bg-white/[0.02] px-3 py-1 rounded">
                {tech}
              </span>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8 font-mono text-xs leading-relaxed text-neutral-300">
          {project.problemStatement && (
            <div className="bg-white/[0.01] border border-white/5 p-4 rounded">
              <span className="block text-[10px] uppercase tracking-widest text-neutral-500 mb-2">// PROBLEM STATEMENT</span>
              <p className="font-sans text-sm font-light text-neutral-300">{project.problemStatement}</p>
            </div>
          )}
          {project.solutionStatement && (
            <div className="bg-white/[0.01] border border-white/5 p-4 rounded">
              <span className="block text-[10px] uppercase tracking-widest text-neutral-500 mb-2">// ARCHITECTURAL SOLUTION</span>
              <p className="font-sans text-sm font-light text-neutral-300">{project.solutionStatement}</p>
            </div>
          )}
        </div>
        ...
        {project.myContribution && project.myContribution.length > 0 && (
          <div className="mb-8">
            <span className="block font-mono text-[9px] uppercase tracking-[0.25em] text-neutral-500 mb-3">// ENGINEERING CONTRIBUTIONS</span>
            <ul className="space-y-2 font-mono text-xs text-neutral-300 list-disc list-inside">
              {project.myContribution.map((item, idx) => (
                <li key={idx} className="leading-relaxed"><span className="font-sans text-neutral-300">{item}</span></li>
              ))}
            </ul>
          </div>
        )}
        ...
        {project.keyFeatures && project.keyFeatures.length > 0 && (
          <div className="mb-8">
            <span className="block font-mono text-[9px] uppercase tracking-[0.25em] text-neutral-500 mb-3">// CORE SYSTEM FEATURES</span>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {project.keyFeatures.map((feat, idx) => (
                <div key={idx} className="border border-white/5 bg-white/[0.01] p-3 rounded font-mono text-[11px] text-neutral-300">
                  {feat}
                </div>
              ))}
            </div>
        ...
        {project.type === 'OFFICE PROJECT' && project.hasScreenshots && (
          <div className="mb-8">
            <span className="block font-mono text-[9px] uppercase tracking-[0.25em] text-neutral-500 mb-3">// SYSTEM TELEMETRY PREVIEW</span>
            <div className="w-full h-48 sm:h-64 border border-dashed border-white/15 bg-white/[0.01] rounded flex flex-col items-center justify-center p-6 text-center font-mono">
              ...
              <span className="text-xs text-neutral-400 tracking-widest mb-1">CONFIDENTIAL DATA REDACTED</span>
              <span className="text-[10px] text-neutral-600">INTERNAL OFFICE PREVIEW // SCREENSHOTS SANITIZED</span>
            </div>
          </div>
        )}
