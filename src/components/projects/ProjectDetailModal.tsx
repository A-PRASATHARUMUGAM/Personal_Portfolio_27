...
import { ProjectCover, ACCENT } from "./ProjectCover";
export { ProjectCover, ACCENT };
  ...
  return createPortal(
    <AnimatePresence>
      {project && accent && (
        <motion.div
          key="backdrop"
          className="fixed inset-0 z-[120] flex items-center justify-center p-3 sm:p-6 lg:p-8 bg-black/85 backdrop-blur-md overflow-y-auto overscroll-contain"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        ...
        >
          <motion.div
            key="panel"
            className={`relative w-full max-w-3xl my-auto max-h-[88vh] overflow-y-auto rounded-2xl border ${accent.ring} bg-[#0b0d10] text-slate-100 shadow-[0_0_80px_rgba(0,0,0,0.95)] overscroll-contain focus:outline-none`}
            tabIndex={-1}
            initial={
          ...
          >
            {/* Hero cover */}
            <div className="relative h-48 sm:h-64 w-full overflow-hidden rounded-t-2xl flex-shrink-0">
              <ProjectCover project={project} />
              <div className="absolute inset-0 bg-gradient-to-t from-[#0b0d10] via-[#0b0d10]/30 to-transparent" />

              <button
                onClick={onClose}
                className="absolute top-4 right-4 z-20 flex h-9 w-9 items-center justify-center rounded-full border border-white/20 bg-black/60 text-slate-200 backdrop-blur-md transition-all hover:border-white/50 hover:bg-black/90 hover:text-white hover:scale-110 active:scale-95 cursor-pointer"
                aria-label="Close dialog"
              >
                <X className="h-4 w-4" />
              </button>

              <div className="absolute bottom-4 left-5 right-5 sm:left-6 sm:right-6 flex items-center justify-between font-mono text-[10px] tracking-[0.2em] text-neutral-300">
            ...
            <div className="p-5 sm:p-8">
              ...
              <div className="mt-8 flex flex-wrap items-center gap-3 border-t border-white/10 pt-6 text-xs">
                ...
                <button
                  onClick={onClose}
                  className="ml-auto rounded-lg border border-white/20 px-4 py-2 text-neutral-300 transition-colors hover:border-white hover:text-white cursor-pointer"
                >
                  Close
                </button>
              </div>
